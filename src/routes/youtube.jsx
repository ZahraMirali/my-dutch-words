...
export default function Youtube() {
  ...
  const copyTenToClipboard = () => {
    if (copyIndex !== "" && objectsArray[copyIndex]) {
      const textToCopy = objectsArray
        .slice(copyIndex, parseInt(copyIndex) + 10)
        .map((object) => {
          const word = object.word || "";
          const meaning = object.meaning;
          const examples = Array.from({ length: 3 }, () => object.example).join(
            "\n"
          );
          return `${word}\n${object.example}\n${meaning}\n${examples}\n.\n`;
        })
    ...
    <div>
      ...
      <div className="sticky top-0 z-10 bg-white gap-4 flex-wrap w-full py-2 shadow-md border-b border-gray-300 hidden md:flex">
        ...
        <input
          type="number"
          value={copyIndex}
          onChange={(e) => setCopyIndex(e.target.value)}
          className="border border-gray-300 p-1 rounded ml-2"
        />
        <button
          onClick={copyTenToClipboard}
          className="bg-violet-500 text-white py-1 px-2 rounded ml-2"
        >
          Copy
        </button>
      </div>
